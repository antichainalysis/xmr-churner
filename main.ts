...
const argv = yargs(hideBin(process.argv))
    ...
    .option("churnRange", {
        alias: "cr",
        type: "array",
        description: "Random number of times to perform churn by given range - Churn range, e.g., --churnRange, -cr 3,7",
...
const churnRange: [number, number] = argv.churnRange.length === 2 ? argv.churnRange as [number, number] : [3, 7];
...
async function main() {
    ...
    const [minRange, maxRange] = calculateAmuRange(totalBalance).map(value => value * 1e12);
